...
async function get(user,offset){
  var response = await fetch(`https://scratchproxy.deta.dev/users/${user}/following?limit=40&offset=${offset}`);
...
async function getCount(user,pages){
    ...
    if(status == 404){
      alert(`It seems like "${user}" doesn't exist or he got eaten by a llama...`);return(false)
    }else{
      var data = await response.text();
      data = JSON.parse(data);
      if(Object.hasOwn(data,"statistics")){count = data.statistics.following;}
      else{count = 10}
      console.log(count);
...
function ready(user){
  ...
  download(`${user}'s_followed.txt`,RESULT.join("\n"),html_download_txt);
  download(`${user}'s_followed.csv`,RESULT.join(","),html_download_csv);
  download(`${user}'s_followed.json`,JSON.stringify(RESULT),html_download_json);
}
...
//Scratch api - https://www.en.scratch-wiki.info/wiki/Scratch_API#GET_.2Fusers.2F.3Cusername.3E.2Fmessages.2Fcount
